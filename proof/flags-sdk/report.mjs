...
// Same browser the runner used. `PROOF_CHROME` points at a real Chrome binary
// when Playwright's own pinned build isn't installable (its CDN currently
// answers 400 for every build). Without this the report writer silently falls
// back to "image embedding skipped" / "cursor glyphs skipped" and ships a
// REPORT.html whose media doesn't render.
const LAUNCH = process.env.PROOF_CHROME ? { executablePath: process.env.PROOF_CHROME } : {};
